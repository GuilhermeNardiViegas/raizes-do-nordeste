(function () {
  ...
  const defaultStatuses = [
    ...
    },
    {
      ...
      description: "O Pagemento foi aceito."
    },
    {
      key: "kitchen",
      title: "Em preparo",
  ...
  const App = {
    ...
    migrateSeedData() {
      ...
      const migratedUsers = users.map((user) => {
        if (user.email && user.email.toLowerCase() === "teste@email.com") {
          return {
            ...user,
            name: "Guilherme",
        ...
        if (user.email && user.email.toLowerCase() === "guilherme@email.com") {
          return {
            ...user,
            name: "Guilherme",
            phone: "(11) 95990-1295",
        ...
        if (user.email && user.email.toLowerCase() === "admin@teste.com") {
          return {
            ...user,
            name: "Admin",
      ...
      if (session && session.email && session.email.toLowerCase() === "teste1@email.com") {
    ...
    getDisplayName() {
      const session = this.getSession();
      return session && session.name ? session.name : "Guilherme";
    },
    ...
    normalizeOrder(order) {
      ...
      return {
        ...
        customer: {
          ...
          email: customer.email || "raizesdonordeste@email.com",
          phone: customer.phone || "11 0000-0000"
        },
        payment: {
    ...
    createOrder(paymentData) {
      ...
      const order = this.normalizeOrder({
        ...
        customer: {
          name: String(safePaymentData.customerName || session.name || "Cliente Raíz").trim(),
        ...
        payment: {
          ...
          provider: "Gateway Nubank",
          transactionId: `Nubank${Math.floor(100000 + Math.random() * 900000)}`,
          ...
          <div class="site-footer">
            <div class="footer-shell">
              <div class="footer-columns">
                <div>
                  <h2>Bem vindo!</h2>
                  <p>Sabores reais do nosso Nordeste!</p>
                </div>
                <div>
                  <h2>Privacidade e Segurança</h2>
                  <p>Consentimento explícito e clareza no uso dos dados!</p>
                </div>
            </div>
          </div>
        `;
    ...
    renderConsentBanner() {
      ...
      banner.innerHTML = `
        ...
        <p>Usamos somente os dados necessários para autenticação e comunicação do atendimento, com consentimento do usuário.</p>
    ...
    handleProfilePage() {
      ...
      if (clearButton) {
        clearButton.addEventListener("click", () => {
          if (!window.confirm("Deseja limpar os dados?")) {
            return;
          }
